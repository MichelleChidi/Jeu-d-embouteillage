...
public class CreateFile {
	...
	public CreateFile(CreateCardModel m) {
		model = m;
		createView();
		placeComponent();
		createController();
	...
	public void createController() {
		frame.addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				int reponse = JOptionPane.showConfirmDialog(frame,
						"Voulez-vous quitter sans donner de nom à votre niveau ?", "Confirmation",
		...
		confirmer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (nom.getText() != null) {
					File monFichier = new File("niveaux/perso/" + nom.getText() + ".lvl");
					...
					int reponse = JOptionPane.showConfirmDialog(frame,
							...
							JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
					if (reponse == JOptionPane.YES_OPTION) {
						Card card = new StdCard(new File("niveaux/perso/" + model.getNbFileWithoutName() + ".lvl"));
						try {
							card.save(model.getAllVehicles());
						} catch (IOException e1) { e1.printStackTrace(); }
